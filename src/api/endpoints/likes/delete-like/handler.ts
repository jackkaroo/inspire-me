...
export async function handler(req: AuthenticatedRequest, res: Response): Promise<void> {
  ...
  const subscription = await prisma.like.delete({
    where: {contentId_userId: {contentId: parseInt(id), userId: userId}},
  });

  logger.info(`Deleted like for user ${userId} from DB with contentId:`, {message: id});

  res.send(subscription);
}

export const deleteLikeHandler = wrapHandler(handler, {params: IdParamSchema});
